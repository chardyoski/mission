$(function(){
	...
	// $('.much-more .reduce-btn').hide()
	// $('.much-more #num').hide()
	$('.much-more .num').each(function () {
	...
	$('.much-more .add-btn').click(function () {
	    ...
	    $('.much-more .reduce-btn').click(function () {
        var booksid = $(this).attr('booksid')
        var $that = $(this)
        ...
        $.get('/mymission/subcart/', {'booksid':booksid}, function (response) {
			...
			// $('#cart').click(function () {
			// 	$.get('/mymission/addcart/',function (response) {
			// 		if (response.status==-1){
			// 			window.open('/mymission/denglu',target='_self')
			// 		}
			//
